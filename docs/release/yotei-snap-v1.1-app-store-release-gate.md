...
| Required next build | `6` (stable Xcode Cloud archive and App Store Connect processing passed; exact TestFlight device smoke pending) |
...
| Build 5 App Store metadata | PASS (historical) | Seven Japanese 1320 × 2868 RGB screenshots, current support/privacy URL, the Build 5 review note, manual release, existing ratings, and no-login state were saved for the invalidated submission attempt; the Build 6 reviewer-facing note remains pending save until the exact TestFlight device smoke passes |
...
- Build 6 processing is `PASS` from TestFlight `提出準備完了` and bounded Apple-mail evidence. The exact-cloud-binary TestFlight installation and real-iPhone smoke remain `PENDING`: App Store Connect currently contains zero existing TestFlight testers, and the approved scope forbids creating a tester, adding a new email address, or creating a group without further authorization.
- Reviewer-note save, App Review submission, and approval remain separate pending gates and have not been performed.
